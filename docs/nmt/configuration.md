...
| `nmt.verbose` | `false` | enable verbose llama.cpp loader/runtime logs |
...
sentence-level translation, one request per context (`bs=1`, like ASR/TTS).
...
The optional `riva_server` adapter implements `RivaTranslation.TranslateText`
and `ListSupportedLanguagePairs`.
...
When ASR is loaded, the same adapter also exposes
`StreamingTranslateSpeechToText`; loading TTS enables
`StreamingTranslateSpeechToSpeech`. Both use the shared `SpeechTranslator`
composition rather than separate transport-owned inference paths.
