...
constexpr int kHalfTaps = 16;  // 32-tap Hann-windowed sinc.
...
AudioResampler::emit_available(bool final, std::vector<float>* output) {
    ...
    while (true) {
        if (final && next_output_ >= final_count)
            break;
        const long double source_position = static_cast<long double>(next_output_) *
                                            static_cast<long double>(input_rate_) /
                                            static_cast<long double>(output_rate_);
        const int64_t center = static_cast<int64_t>(std::floor(source_position));
        if (!final && center + kHalfTaps >= static_cast<int64_t>(total_input_))
            break;
        output->push_back(filtered_sample(source_position));
        ++next_output_;
    }
}

float
AudioResampler::filtered_sample(long double source_position) const {
    const int64_t center = static_cast<int64_t>(std::floor(source_position));
    const long double rate_ratio =
        static_cast<long double>(output_rate_) / static_cast<long double>(input_rate_);
    // Leave a small transition band below the destination Nyquist frequency.
    const long double cutoff = rate_ratio < 1.0L ? rate_ratio * 0.94L : 1.0L;
    long double weighted = 0.0L;
    long double weight_sum = 0.0L;

    for (int64_t index = center - kHalfTaps + 1; index <= center + kHalfTaps; ++index) {
        const long double distance = source_position - static_cast<long double>(index);
        ...
        const long double weight = cutoff * sinc(cutoff * distance) * window;
        weighted += static_cast<long double>(input_sample(index)) * weight;
        weight_sum += weight;
    }
...
AudioResampler::compact_history() {
    if (input_.empty())
        return;
    const long double next_position = static_cast<long double>(next_output_) *
                                      static_cast<long double>(input_rate_) /
                                      static_cast<long double>(output_rate_);
    const int64_t earliest_needed = static_cast<int64_t>(std::floor(next_position)) - kHalfTaps + 1;
    if (earliest_needed <= static_cast<int64_t>(input_start_))
        return;
    const uint64_t discard64 =
        std::min<uint64_t>(static_cast<uint64_t>(earliest_needed) - input_start_, input_.size());
...
Pcm16Resampler::process(const uint8_t* input, size_t bytes, std::vector<uint8_t>* output) {
    ...
    std::vector<float> decoded(bytes / 2);
    constexpr float scale = 1.0f / 32768.0f;
    for (size_t i = 0; i < decoded.size(); ++i) {
        const uint16_t bits =
            static_cast<uint16_t>(input[i * 2]) | (static_cast<uint16_t>(input[i * 2 + 1]) << 8);
        decoded[i] = static_cast<float>(static_cast<int16_t>(bits)) * scale;
    }

    std::vector<float> converted;
    resampler_.process(decoded.data(), decoded.size(), &converted);
    append_pcm(converted, output);
}
