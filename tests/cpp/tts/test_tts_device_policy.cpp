...
using Device = nemo_speech::tts::TtsCliDevice;
...
test_cpu_forces_all_components_to_cpu() {
    Config config;
    apply_tts_device_policy(config, Device::Cpu);
...
test_accelerator_preserves_codec_cpu_override() {
    ...
    apply_tts_device_policy(config, Device::Accelerator);
...
void
test_accelerator_clears_magpie_cpu_override() {
    Config config;
...
test_cuda_preserves_codec_cpu_override() {
    ...
    apply_tts_device_policy(config, Device::Cuda);
...
void
test_cuda_clears_magpie_cpu_override() {
    Config config;
    config.magpie_cpu = true;

    apply_tts_device_policy(config, Device::Cuda);

    require(!config.magpie_cpu, "CUDA device must keep MagpieTTS on CUDA");
}
