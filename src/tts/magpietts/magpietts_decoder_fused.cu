...
static constexpr int DF_XATTN_MAX_KEYS = 64;  // keys per tail block
static_assert(
    DF_XATTN_BLOCKS * DF_XATTN_MAX_KEYS == MAGPIETTS_DECODER_FUSED_MAX_TEXT,
    "fused decoder text window");
...
struct df_args {
    ...
    int prefetch;  // L2-prefetch upcoming weights
...
df_self_attention(const df_args& a, float* kbase, float* vbase, int tile, int nblocks, Smem& sm) {
...
template <class Smem>
static __device__ __forceinline__ void
df_combine_self(const df_args& a, Smem& sm) {
...
template <class Smem>
static __device__ __forceinline__ void
df_cross_q_tail(const df_args& a, int il, int slot, int nblocks, Smem& sm, int* s_flag) {
    const int tid = threadIdx.x;
    const int warp = tid >> 5;
    const int lane = tid & 31;
    const int K = a.E;
...
template <class Smem>
static __device__ __forceinline__ void
df_cross_attention(const df_args& a, int il, int slot, Smem& sm) {
    const int tid = threadIdx.x;
    const int warp = tid >> 5;
    const int lane = tid & 31;
    const int CD = a.CD;
    // real text rows only (padded rows have zero weight)
    const int keys_per = (a.T + DF_XATTN_BLOCKS - 1) / DF_XATTN_BLOCKS;
    const int j0 = slot * keys_per;
    const int n_keys = min(keys_per, a.T - j0);  // may be <= 0 for the last slots
    const df_layer& L = a.layers[il];
    ...
    __syncthreads();
    float local_max = -INFINITY;
    for (int jj = warp; jj < n_keys; jj += LTF_WARPS) {
        float s = 0.f;
...
#pragma unroll
    ...
    __syncthreads();
    float local_sum = 0.f;
    for (int jj = tid; jj < n_keys; jj += LTF_THREADS) {
...
#pragma unroll
    ...
    for (int d = tid; d < CD; d += LTF_THREADS) {
        float acc = 0.f;
        for (int jj = 0; jj < n_keys; ++jj) acc += sc[jj] * V[(size_t)jj * CD + d];
        out[2 + d] = acc;
    }
    if (tid == 0) {
        out[0] = m;
...
template <class Smem>
static __device__ __forceinline__ void
df_combine_cross(const df_args& a, int il, int tile, Smem& sm) {
    const int tid = threadIdx.x;
    const int CD = a.CD;
    float* ctx = sm.u.g.s_x;
    float M = -INFINITY;
...
#pragma unroll
    ...
    if (tile == 0 && a.alignment && (a.layers[il].flags & 2)) {
        for (int j = tid; j < a.T; j += LTF_THREADS)
...
template <class Smem>
static __device__ __forceinline__ void
df_final(const df_args& a, int tile, int nblocks, Smem& sm) {
    const int tid = threadIdx.x;
    const int warp = tid >> 5;
    const int lane = tid & 31;
    const int E = a.E;
...
df_kernel(const df_args a) {
    ...
    ltf_chain_smem<Tier>& sm = *reinterpret_cast<ltf_chain_smem<Tier>*>(df_dyn_smem);
    ...
    for (int il = 0; il < a.n_layers; ++il) {
        ...
        // prefetch weights staged one or two phases later
        if (a.prefetch) {
            ltf_chain_prefetch_l2(L.o, E, E, tile);
            if (L.flags & 1)
                ltf_chain_prefetch_l2(L.co, a.CD, E, tile);
            ltf_chain_prefetch_l2(L.ff1, E, a.NFF, tile);
        }
        ...
        sync();
        if (a.prefetch)
            ltf_chain_prefetch_l2(L.ff2, a.NFF, E, tile);
        ...
        sync();
        if (a.prefetch && il + 1 < a.n_layers)
            ltf_chain_prefetch_l2(a.layers[il + 1].qkv, E, 3 * E, tile);
...
magpietts_decoder_fused_create(
    ...
    if (!ltf_kernel_arch_supported(
            reinterpret_cast<const void*>(df_kernel<ltf_chain_tier_large>))) {
        ltf_set_error(
    ...
    // Same tiling as the LT chain; the cross-attention query runs on the DF_XATTN_BLOCKS tail
    // blocks, two rows per warp.
    auto fits = [&](auto tier) {
        using Tier = decltype(tier);
        const int grid = Tier::grid;
        return E % grid == 0 && (E / grid) % 4 == 0 && NFF % grid == 0 && (NFF / grid) % 16 == 0 &&
               ltf_chain_phase_fits<Tier>(3 * E, E, Tier::r_qkv) &&
               ltf_chain_phase_fits<Tier>(E, E, Tier::r_o) &&
               ltf_chain_phase_fits<Tier>(E, CD, Tier::r_o) &&
               ltf_chain_phase_fits<Tier>(NFF, E, Tier::r_ff1) &&
               ltf_chain_phase_fits<Tier>(E, NFF, Tier::r_o) && (NFF >> 5) <= 96 &&
               DF_XATTN_BLOCKS <= grid && CD % (2 * DF_XATTN_BLOCKS) == 0 &&
               CD / DF_XATTN_BLOCKS <= 2 * LTF_WARPS;
    };
    int grid = 0;
    if (ltf_chain_tier_usable<ltf_chain_tier_large>(
        ...
        char msg[128];
        snprintf(
            msg, sizeof(msg), "fused decoder: no persistent tier fits (%d SMs, needs %d+)", sms,
            ltf_chain_tier_small::grid / LTF_CHAIN_BLOCKS_PER_SM);
        ltf_set_error(error, error_size, msg);
        magpietts_decoder_fused_free(f);
        return nullptr;
    }
    f->grid = grid;
    ...
    {
        // Prefetch only if L2 holds the ~1 layer in flight with room to spare.
        int l2 = 0;
        cudaDeviceGetAttribute(&l2, cudaDevAttrL2CacheSize, device);
        const size_t layer_q8 = ((size_t)4 * E * E + 2 * (size_t)CD * E + 2 * (size_t)NFF * E);
        a.prefetch = (size_t)l2 >= 2 * (layer_q8 + layer_q8 / 16) ? 1 : 0;
    }
    a.ln_eps = w.ln_eps;
...
bool
magpietts_decoder_fused_exclusive(const magpietts_decoder_fused* f) {
    return f && f->grid == ltf_chain_tier_small::grid;
}
...
magpietts_decoder_fused_timing_report() {
    ...
    static const char* const slots[] = {
        "qkv", "self-attention", "o-proj + cross-attn tail", "cross-o", "ff1", "ff2"};
    ltf_timing_report("fused decoder (per layer)", slots, 6);
#endif
}
...
magpietts_decoder_fused_step(
    ...
    if (!f || f->grid <= 0 || !s.tokens || !s.hidden_cond || !s.hidden_uncond || !s.prior ||
        ...
        s.ring_head >= f->cache.cache_len || s.text_len < 0 ||
        s.text_len > f->cache.text_capacity) {
    ...
    a.T = s.text_len > 0 ? s.text_len : f->cache.text_capacity;
    a.prior = s.prior;
    a.mask = s.mask;
    ...
    if (f->grid == ltf_chain_tier_large::grid) {
        df_kernel<ltf_chain_tier_large>
            <<<f->grid, LTF_THREADS, sizeof(ltf_chain_smem<ltf_chain_tier_large>), stream>>>(a);
    } else {
        df_kernel<ltf_chain_tier_small>
            <<<f->grid, LTF_THREADS, sizeof(ltf_chain_smem<ltf_chain_tier_small>), stream>>>(a);
    }
